...
trait Solver extends GameDef:
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] = {
            val next = 
              for 
                current <- initial
                n <- newNeighborsOnly(neighborsWithHistory(current._1, current._2), explored)
              yield n
   ...
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from(Seq((startBlock, Nil)).to(LazyList), explored = Set(startBlock))

  /**
   * Returns a lazy list of all possible pairs of the goal block along
   * with the history how it was reached.
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart.takeWhile(p => done(p._1))
  ...
  lazy val solution: List[Move] = {
    val r = pathsFromStart.filter(p => done(p._1))
    r.head._2.reverse
  }
