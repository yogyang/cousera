...
class HuffmanSuite extends munit.FunSuite:
  ...
  test("decode and encode should identify") {

    def testFun(s: String) = {
      val codeTree = createCodeTree(s.toList)
      val encode = quickEncode(codeTree)(s.toList)
      val de = decode(codeTree, encode)
      assertEquals(de, s.toList)
    }

    testFun("Test description: decode and encode some longer text should be identity ")
  }

  test("combine should still maintain order") {
    new TestTrees :
      val r = combine(List(Leaf('a',2), Leaf('b',3), Leaf('c', 4), Leaf('d', 7)))
      println(r)
  }
